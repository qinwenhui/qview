...
use qview::edit::{EditBuffer, LineEditor};
use qview::file::LineIndex;
...
fn temp_log(contents: &str) -> std::path::PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("qview-edit-{}.log", nanos));
...
fn line_view_resolve_basic() {
    ...
    let path = temp_log("a\nb\nc");
    let mmap = qview::file::MmapBackend::open(&path).unwrap();
    let builder = qview::file::IndexBuilder::new(mmap.clone());
    let offsets = builder.build_with_progress(|_, _| {}).unwrap();
    let index = LineIndex::from_vec(offsets, mmap.size());

    let edits = EditBuffer::new();
    let view = qview::edit::LineView::new(&mmap, &index, &edits, &edits.mapping);
